...
void CLookUpQueue::createCompletionStats(Request *req)
{
    _track = req->track();
    _sector = req->sector();
    _entr = req->time();
...
void CLookUpQueue::doThisWhenIdle(EventQueue* eventQueue , Request* req )
{
...
void CLookUpQueue::processSingleQueue(Request *req)
{
    createCompletionStats(req);
}
...
int CLookUpQueue::dist(int current_track, int dest_track)
{
...
int CLookUpQueue::Rotdist(int current_sector, int dest_sector)
{
...
void CLookUpQueue::addRequest(Request *request)
{
    ...
    if ( request->track() < head->request()->track() )
    {
        ...
        if ( currentPtr != nullptr && request->track() == currentPtr->request()->track() )
        {
        ...
        if (currentPtr != nullptr && currentPtr->request()->track() > head->request()->track())
        {
